...
def calculate_outstanding_balance(p, n, r, m):
  return round((p*((1 +( r/100))**n) - ((1+(r/100))**m))/((1+(r/100))**n - 1),2)

st.title('Improvised EMI Calaculator')
...
r = roi/12
m = st.sidebar.slider('Period after which the Outstanding Loan Balance is calculated (in months)',1,n,(0.25,0.5))
